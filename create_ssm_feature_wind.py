...
def main(inpath, outpath):
    ...
    with open("utils/SH_pool.txt", "r") as f:
        stocks = [line.strip() for line in f.readlines()]
    # paths = glob(os.path.join(inpath, "*.csv"))
    pathlib.Path(outpath).mkdir(parents=True,exist_ok=True)
    paths = [os.path.join(inpath, f"{stock}.csv") for stock in stocks if
             os.path.exists(os.path.join(inpath, f"{stock}.csv"))]
    for path in paths:
        name = os.path.basename(path)

        df = pd.read_csv(path, index_col=0)
        df.rename({'Open': 'open', 'High': 'high', 'Low': 'low', 'Close': 'close', 'Adj Close': 'adjclose', 'Volume': 'volume'}, axis=1, inplace=True)
        cal_feature(df)
        cal_target(df)
...
if __name__ == '__main__':
    ...
    main("Dataset/沪深数据/raw",
         "Dataset/沪深数据/ssm_feature")