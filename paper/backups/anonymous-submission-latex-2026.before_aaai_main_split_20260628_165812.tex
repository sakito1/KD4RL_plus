...
\]
where \(\alpha\in[0,1]\) controls the refinement strength. Further implementation details of the inner architecture and refinement construction are deferred to Appendix~\ref{app:hier_inner}.
...
\begin{algorithm}[t]
\caption{End-to-End Staged Training of \textit{CMTFlow}}
\label{alg:cmtflow_training}
\algsmall
\begin{algorithmic}[1]
\REQUIRE Market record \(\mathcal{H}\), reference horizon \(H_{\mathrm{ref}}\), cost model, and parameters \(\theta^{\mathrm{out}},\theta^{\mathrm{in}},\theta^{\mathrm{ctrl}}\).
\ENSURE Optimized outer actor, inner actor, and controller.
\STATE \textbf{Stage 1: Outer base-portfolio warmup.}
\STATE Build fixed segments \(\mathcal{S}_{\mathrm{ref}}=\{[\tau_k,\tau_k+H_{\mathrm{ref}})\}_k\).
\FOR{each segment boundary \(\tau_k\in\mathcal{S}_{\mathrm{ref}}\)}
\STATE Encode \(s^{\mathrm{out}}_{\tau_k}\) and sample candidate base \(\mathbf{w}^{\mathrm{cand}}_{\tau_k}\).
\STATE Roll the base with drift and costs; compute segment return \(R^{\mathrm{out}}_k\).
\STATE Update \(\theta^{\mathrm{out}}\) by PPO plus the outer auxiliary return loss.
\ENDFOR
\STATE \textbf{Stage 2: Inner daily refinement.}
\STATE Load the best outer checkpoint as the active base generator.
\FOR{each trading day \(t\) inside a fixed segment}
\STATE Encode \(s^{\mathrm{in}}_t\) with the active base and support mask.
\STATE Sample \(\mathbf{w}^{\mathrm{in}}_t\) and execute \(\mathbf{w}_t=(1-\alpha)\mathbf{b}_t+\alpha\mathbf{w}^{\mathrm{in}}_t\).
\STATE Reward relative daily improvement over the base; update \(\theta^{\mathrm{in}}\) by PPO.
\ENDFOR
\STATE \textbf{Stage 3: Fixed-HRL joint stabilization.}
\STATE Keep the controller inactive; jointly update \(\theta^{\mathrm{out}},\theta^{\mathrm{in}}\) and save the best HRL backbone.
\STATE \textbf{Stage 4: Controller policy-gradient optimization.}
\FOR{each controller rollout \(\xi\)}
\FOR{each trading day \(t\) in \(\xi\)}
\STATE Form \(s_t^{\mathrm{ctrl}}\) from the recent market tensor, drifted holding, candidate base, holding state, and action-comparison features.
\STATE Sample \(g_t\sim\pi_{\theta^{\mathrm{ctrl}}}(\mathrm{hold/switch}\mid s^{\mathrm{ctrl}}_t)\).
\STATE Set \(\mathbf{b}_t\leftarrow\mathbf{w}_t^{\mathrm{cand}}\) if \(g_t=\mathrm{switch}\) or the age cap is reached; otherwise set \(\mathbf{b}_t\leftarrow\tilde{\mathbf{b}}_t\).
\STATE Refine the active base with the inner actor and execute the portfolio.
\ENDFOR
\STATE Compare controlled path \(C_\xi\) with fixed reference path \(B_\xi\).
\STATE Update \(\theta^{\mathrm{ctrl}}\) by policy gradient and auxiliary rollout targets. \COMMENT{no switch labels}
\ENDFOR
\STATE \textbf{Stage 5: Controller-active end-to-end finetuning.}
\STATE Load the best controller checkpoint and jointly finetune all modules with daily controller decisions.
\STATE Select the final checkpoint under the same controller-active validation protocol used for testing.
\end{algorithmic}
\end{algorithm}

Algorithm~\ref{alg:cmtflow_training} summarizes the staged training schedule used for the three trainable modules; the appendix provides the corresponding rollout rewards and implementation details.
...
Controller min-hold lock & disabled & disabled \\
Controller max-hold cap & 30 days & 30 days \\
...
We compare \textit{CMTFlow} with traditional online portfolio strategies and deep RL baselines under the same long-only, cost-aware evaluation setting. Traditional and online methods include Buy\&Hold, Markowitz \citep{MVO}, UCRP \citep{UCRP}, Anticor \citep{ANTICOR}, OLMAR \citep{OLMAR}, and WMAMR \citep{WMAMR}. Deep RL baselines include AlphaStock \citep{alphastock}, DeepAries \citep{deeparies2025}, and DeepTrader \citep{6}. To avoid mixing incompatible runs, Table~\ref{tab:main} reports only baseline records whose final metrics can be matched to the same market split and evaluation protocol. Cumulative-wealth plots require the stricter condition that a stored or replayed daily portfolio path reproduces the corresponding metric within tolerance, so Figures~\ref{fig:main_equity_nas} and~\ref{fig:main_equity_sh} visualize the matched trajectory subset rather than every metric-only baseline record.
...
\begin{table}[t]
\centering
\caption{Ablation study results for policy components and fixed-window controllers. Values are percentages except Sharpe and CR. Best and second-best results in each market-metric column are bolded and underlined, respectively.}
\label{tab:ablation}
\footnotesize
...
& TR\(\uparrow\) & Sharpe\(\uparrow\) & MDD\(\downarrow\) & CR\(\uparrow\) \\
\midrule
\textit{CMTFlow} & \best{265.53} & \best{1.15} & \best{18.62} & \best{1.42} & \second{204.99} & \second{1.14} & 22.78 & \second{1.09} \\
\midrule
Outer-only & 220.42 & 1.09 & 32.09 & 0.74 & 147.05 & 0.94 & 20.99 & 0.99 \\
Outer + Inner & 227.43 & \second{1.11} & 31.73 & 0.76 & 158.99 & 0.99 & 20.85 & 1.04 \\
Outer + Controller & \second{237.50} & 1.09 & \second{21.24} & \second{1.18} & \best{237.77} & \best{1.22} & 23.29 & \best{1.16} \\
\midrule
Fixed 5d & 219.84 & 1.03 & 25.56 & 0.95 & 103.69 & 0.77 & \second{19.96} & 0.84 \\
Fixed 10d & 199.65 & 0.98 & 28.24 & 0.81 & 106.08 & 0.79 & 21.70 & 0.78 \\
Fixed 20d & 176.03 & 0.93 & 25.72 & 0.83 & 98.23 & 0.75 & \best{19.92} & 0.81 \\
Fixed 30d & 227.43 & \second{1.11} & 31.73 & 0.76 & 158.99 & 0.99 & 20.85 & 1.04 \\
Fixed 60d & 170.88 & 0.95 & 29.96 & 0.69 & 129.73 & 0.89 & 21.25 & 0.90 \\
\bottomrule
\end{tabular}
}
\end{table}

Table~\ref{tab:ablation} identifies the controller as the main source of adaptive behavior. On Nasdaq-100, adding the controller to the outer actor improves total return from 220.42\% to 237.50\%, reduces MDD from 32.09\% to 21.24\%, and raises CR from 0.74 to 1.18; the full model further reaches 265.53\% return, 18.62\% MDD, and the best CR of 1.42. On CSI-300, Outer + Controller achieves the highest ablation return, Sharpe ratio, and CR, while the full model keeps a slightly lower MDD than Outer + Controller and outperforms all fixed-window controllers in total return. This contrast clarifies the roles of the modules: the controller is the dominant mechanism for dynamic revision, whereas the inner actor is a local weight-refinement module whose return contribution is market-dependent rather than uniformly positive.
...
We examine the controller from three perspectives that directly test whether the learned revision policy has economic meaning: representative switch cases, comparison with multiple fixed holding windows, and aggregate counterfactual/probability statistics.

\begin{center}
\begin{minipage}{\columnwidth}
\centering
\includegraphics[width=\columnwidth]{figures/explainability/controller_switch_case_nas.png}
\captionof{figure}{\textbf{Nasdaq-100 switch case.} Frozen 30-day counterfactuals compare keeping the old base with switching to the controller-selected new base.}
...
\captionof{figure}{\textbf{CSI-300 switch case.} The controller-selected switch avoids the old base continuation's subsequent return loss and drawdown.}
\label{fig:controller_case_sh}
\end{minipage}
\end{center}

Figures~\ref{fig:controller_case_nas} and~\ref{fig:controller_case_sh} show switch cases in which continuing the previous base portfolio would have led to weaker future outcomes. Each case freezes the two alternatives at the switch date and compares the old-base continuation with the controller-selected new base over the same 30-trading-day horizon, so the comparison is not affected by later switches. Day 0 is the selected actual switch; the endpoint markers in the return panel show the 30-day terminal returns, the markers in the drawdown panel show each path's maximum drawdown point, and the bottom panel reports only the day-0 decision evidence. In the Nasdaq case, the key switch on 2021/04/19 improves the counterfactual return from \(-1.91\%\) to 0.52\% and reduces future MDD from 8.82\% to 5.51\%. In the CSI-300 case, the key switch on 2021/07/07 improves the counterfactual return from \(-10.29\%\) to 10.25\% and reduces future MDD from 13.71\% to 8.23\%. These cases show the economic meaning of the controller: selected switches can terminate a deteriorating holding state before the old base portfolio fully realizes the subsequent drawdown.

\begin{center}
\begin{minipage}{\columnwidth}
\centering
\includegraphics[width=\columnwidth]{figures/explainability/fixed_window_comparison_nas.png}
\captionof{figure}{\textbf{Nasdaq-100 learned versus fixed holding windows.} Fixed-window policies rebalance every 5/10/20/30/60 trading days, while the learned controller evaluates the holding state daily.}
\label{fig:fixed_window_nas}
\end{minipage}
\end{center}
...
\begin{minipage}{\columnwidth}
\centering
\includegraphics[width=\columnwidth]{figures/explainability/fixed_window_comparison_sh.png}
\captionof{figure}{\textbf{CSI-300 learned versus fixed holding windows.} The comparison tests whether a manually selected holding period can replace the state-dependent controller.}
\label{fig:fixed_window_sh}
\end{minipage}
\end{center}

Figures~\ref{fig:fixed_window_nas} and~\ref{fig:fixed_window_sh} test whether a manually fixed holding period can replace the learned controller. On Nasdaq-100, \textit{CMTFlow} obtains 265.53\% return, 1.15 Sharpe, 18.62\% MDD, and 1.42 CR, outperforming all fixed 5/10/20/30/60-day windows on the reported risk-return metrics. On CSI-300, \textit{CMTFlow} also achieves higher return, Sharpe, and CR than all fixed-window variants; although several fixed windows have slightly lower MDD, they do so with much lower terminal return. These comparisons show that the controller's benefit is not merely caused by choosing a good constant rebalancing period, but by adapting the revision time to the observed holding state.

As an aggregate counterfactual check, we also compare all free switches against frozen old-holding continuations from the same switch date to the same remaining horizon. This avoids contamination from later switches within the same holding period. The comparison covers all free switches in the final traces: 231/231 decisions on Nasdaq-100 and 102/102 decisions on CSI-300. On Nasdaq-100, the mean switch return is 3.29\%, the mean old-holding return is 3.24\%, and the positive-gain ratio is 50.22\%. On CSI-300, the mean switch return is 4.43\%, the mean old-holding return is 4.39\%, and the positive-gain ratio is 48.04\%. The average advantage is intentionally interpreted conservatively: many switches are small maintenance decisions rather than large alpha events. The aggregate check shows that switching is not systematically worse than continuing, while the case studies and fixed-window comparison explain where the realized path-level advantage comes from.
...
\captionof{figure}{\textbf{Nasdaq-100 inner actor resonance.} The heatmaps compare future 5-day relative returns, inner tilts, and executed weights; the bottom panel aggregates asset-level tilt-return alignment.}
