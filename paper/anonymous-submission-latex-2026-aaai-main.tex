...
\centering
\includegraphics[width=\linewidth]{figures/main_equity_nas.pdf}
\end{minipage}
\hfill
\begin{minipage}{0.49\textwidth}
\centering
\includegraphics[width=\linewidth]{figures/main_equity_sh.pdf}
\end{minipage}
\caption{\textbf{Cumulative wealth curves on Nasdaq-100 and CSI-300.} The plots show the matched daily trajectory subset whose final metrics reproduce Table~\ref{tab:main} within tolerance.}
\label{fig:main_equity}
\end{figure*}
...
We presented \textit{CMTFlow}, a hierarchical portfolio framework that separates base construction, daily refinement, and learned base revision. The final model follows the same controller-active decision protocol used during evaluation, rather than relying on external switch labels or a fixed rebalancing calendar. Experiments on Nasdaq-100 and CSI-300 show that the learned controller improves the return-risk trade-off against matched baselines and fixed-window variants, while the inner actor provides interpretable within-holding-period tilts around the active base portfolio.
