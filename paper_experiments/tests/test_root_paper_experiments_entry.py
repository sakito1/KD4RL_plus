...
def test_root_entry_delegates_to_canonical_main() -> None:
    entry = load_root_entry()

    assert entry.main is canonical.main


def test_root_entry_runs_from_repository_root(monkeypatch, tmp_path: Path) -> None:
    entry = load_root_entry()
    repo_root = Path(__file__).resolve().parents[2]
    observed = {}
    monkeypatch.setattr(entry, "main", lambda: observed.setdefault("cwd", Path.cwd()))
