...
def wmamr(prices: pd.DataFrame, window: int = 5, eps: float = 10.0) -> pd.DataFrame:
    """
    Implements the On-Line Weighted Moving Average Mean Reversion (WMAMR) algorithm.

    Parameters:
    - prices: DataFrame of historical prices (index = dates, columns = asset ticks)
    - window: look-back window size for weighted moving average
    ...
    # Pre-compute WMA weights: more recent get higher weight
    # Linear weights: days 1 to window mapped to weights
    w = np.arange(1, window + 1)
    w = w / w.sum()

    for t in range(window, len(prices)):
        # Calculate weighted moving average of past 'window' days
        past_prices = prices.iloc[t - window:t]
        # WMA: sum(weights * prices) along time axis
        wma = (past_prices.values.T * w).T.sum(axis=0)
        # Create DataFrame-like series
        wma_series = pd.Series(wma, index=prices.columns)

        # Predict relative price
        x_pred = wma_series / prices.iloc[t]
    ...
    return pd.DataFrame(weights_history, index=prices.index[window:], columns=prices.columns)
...
def run(cun_path, logger):
    ...
    cun_path = os.path.join(cun_path, "wmamr")
    ...
    # 1) 生成信号权重（基于 close）
    weights_df = wmamr(prices_full, window=config.wmamr_window_size, eps=5.0)
    ...
    for current_pos in range(start_pos, end_pos + 1):
        ...
        if (current_pos - start_pos) % 30 == 0 and day in weights_df.index:
    ...
    logger.info("\n====== 测试区间绩效 ======")
    logger.info("基线策略：wmamr (Close-to-Close)")
    ...
    plt.plot(wealth_index, label="wmamr Portfolio")
