...
echo "gpu_workers=${#gpu_ids[@]} configurations=${#job_markets[@]} total_seed_runs=$total_seed_runs"
...
for gpu_slot in "${!gpu_ids[@]}"; do
  ...
  (
    ...
    for job_index in "${!job_markets[@]}"; do
      ...
      market="${job_markets[$job_index]}"
      k="${job_k_values[$job_index]}"
      seed_string="${job_seed_strings[$job_index]}"
