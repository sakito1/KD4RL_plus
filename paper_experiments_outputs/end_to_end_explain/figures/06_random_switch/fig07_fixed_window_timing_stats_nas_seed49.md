# Dense Fixed Holding-Window Timing Baseline (Nasdaq-100, seed 49)
...
- Learned controller 的 TR 为 265.53%，Sharpe 为 1.15，MDD 为 18.62%，CR 为 1.42。
- 固定窗口中最高 TR 来自 8d，TR 为 336.91%；controller 在 TR 上优于 46/60 个固定窗口。
- 固定窗口中最高 Sharpe 来自 48d，Sharpe 为 1.30；controller 在 Sharpe 上优于 44/60 个固定窗口。
- 固定窗口中最低 MDD 来自 22d，MDD 为 18.63%；controller 在 MDD 上优于 60/60 个固定窗口。
- 固定窗口中最高 CR 来自 8d，CR 为 1.52；controller 在 CR 上优于 57/60 个固定窗口。

注意：Nasdaq-100 上可能存在少数事后挑选的固定窗口（例如 8d）在部分指标上高于 controller。因此论文中不应写成“controller 在所有固定窗口和所有指标上都是第一”。更合理的结论是：controller 不需要事后选择固定窗口，却在固定窗口集合中取得高分位表现，并在关键风险指标上体现出更稳定的控制能力。
