...
class DataGenerator():
    def __init__(self,
        ...
        self.order_set = np.arange(5 * (self.window_len + 1), self.val_idx - 6 * trade_len)
    ...
    def _step(self):
        ...
        # === 新增部分 ===
        # 每个资产的每日累计收益因子 (1+r) 连乘
        # (B, N, trade_len)
        future_cum_ror = np.cumprod((future_return + 1.0), axis=-1)
        ...
        if self.val_mode:
            ...
            done = (self.cursor >= self.test_end_idx-1)
        else:
        ...
        # === 多返回一个 future_cum_ror ===
        return obs, obs_normed, market_obs, market_obs_normed, future_ror, future_p, trade_masks, done, future_cum_ror
    ...
    def reset(self, start_point=None):
        ...
        return obs, obs_normed, market_obs, market_obs_normed, future_ror, trade_masks, done, future_cum_ror
    ...
    def _get_data(self):
        ...
        for i, idx in enumerate(self.cursor):
            ...
            fr = self.__ror_data[:, idx + 1: idx + 1 + self.trade_len]  # shape (N, <=L)
            L = self.trade_len
            N = self.__ror_data.shape[0]
            if fr.shape[1] < L:
                pad_len = L - fr.shape[1]
                fr = np.concatenate([fr, np.zeros((N, pad_len), dtype=fr.dtype)], axis=1)
            future_return[i] = fr
...
class PortfolioSim(object):
    ...
    def _step(self, w0, ror, p, future_cum_ror):
        """
        ...
        # === only long ===
            ...
            daily_return = np.einsum('bnl,bn->bl', future_cum_ror, w0[:, :self.num_assets])

            daily_wealth = dv0[:, None] * (1 - mu0_long)[:, None] * daily_return
...
class PortfolioEnv(object):
    def __init__(self,
        ...
        self.src = DataGenerator(assets_data=assets_data, rtns_data=rtns_data, market_data=market_data,
                                 in_features=in_features, val_idx=val_idx, test_idx=test_idx, test_end_idx=test_end_idx,
    ...
    def step(self, action, p, simulation=False):
        ...
        if simulation:
            ...
            obs, obs_normed, market_obs, market_obs_normed, future_ror, future_p, trade_masks, done1, future_cum_ror = self.src._step()

        ror = self.ror
        cum_ror = self.future_cum_ror
        rewards, info, done2 = self.sim._step(weights, ror, p, cum_ror)

        self.ror = future_ror
        self.future_cum_ror = future_cum_ror
        if self.is_norm:
    ...
    def reset(self):
        self.infos = []
        obs, obs_normed, market_obs, market_obs_normed, future_ror, trade_masks, done, future_cum_ror = self.src.reset()
