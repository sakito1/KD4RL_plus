...
class RLAgent():
    ...
    def evaluation(self, logger=None):
        ...
        val_wealth = np.ones((batch_size, 1), dtype=np.float32)
        rho_record = []
        val_wealth_list = []
        while True:
            steps += 1
            ...
            val_wealth = np.concatenate((val_wealth, info['total_value'][..., None]), axis=-1)
            # 记录 daily_wealth (B,L)
            val_wealth_list.append(info['daily_wealth'])
            states = next_states
        ...
        test_wealth = np.ones((batch_size, 1), dtype=np.float32)
        rho_record = []
        test_wealth_list = []
        while True:
            steps += 1
            ...
            test_wealth = np.concatenate((test_wealth, info['total_value'][..., None]), axis=-1)
            # 记录 daily_wealth (B,L)
            test_wealth_list.append(info['daily_wealth'])
            states = next_states

            if done:
                break

        return test_wealth, val_wealth, test_wealth_list, val_wealth_list
