...
PACKAGED_MARKETS = {
    "nas": {
        "directory": "nasdaq100",
        "actions": "nasdaq100_full_controller_actions.csv",
        "command": "five_stage_training_command.json",
    },
    "sh": {
        "directory": "csi300",
        "actions": "csi300_full_controller_actions.csv",
        "command": "five_stage_training_command.json",
    },
}
...
def discover_model_identity(
    ...
    public_names = PACKAGED_MARKETS[market]
    run_dir = Path(results_root) / public_names["directory"]
    if not run_dir.exists():
        run_dir = Path(results_root) / f"{market}_seed{int(seed)}"
    checkpoint = run_dir / "checkpoints" / "best_model.pth"
    command_json = run_dir / public_names["command"]
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    for market_index, market in enumerate(args.markets):
        ...
        public_names = PACKAGED_MARKETS[market]
        action_path = Path(args.full_actions_root) / public_names["actions"]
