...
OUTPUT_ROOT="${OUTPUT_ROOT:-results/five_stage_training}"
RUN_NAME="${RUN_NAME:-paper_selected_models}"
GPU_ID="${GPU_ID:-0}"
...
if [[ ! -f run_hrl_training.py ]]; then
  echo "run_hrl_training.py not found. Please run this script from the package root." >&2
  exit 1
fi
