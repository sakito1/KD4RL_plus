...
class ControllerDailyAuxPGScriptTests(unittest.TestCase):
    ...
    def test_empty_seed_env_skips_market_loops(self):
        ...
        self.assertIn("NAS seeds:", result.stdout)
        self.assertIn("SH seeds:", result.stdout)
        self.assertIn("lookback60_hold30_daily_pg_balanced_sh90_5ep", result.stdout)
        ...
        self.assertIn("init_exit_bias=-2.5", result.stdout)
        ...
        self.assertIn("Aux warmup: epochs=0", result.stdout)
        self.assertIn("offpolicy=0", result.stdout)
        self.assertIn("replay_epochs=1", result.stdout)
        self.assertIn("switch_adv_coef=0.0", result.stdout)
        ...
        self.assertIn("max_switch_penalty=0.01", result.stdout)
        ...
        self.assertNotIn("diag_thresholds=", result.stdout)
        self.assertNotIn("test_thresholds=", result.stdout)
        self.assertNotIn("Daily aux-PG controller run:", result.stdout)

    def test_controller_uses_single_eval_threshold_without_sweeps(self):
        with tempfile.TemporaryDirectory(prefix="controller-script-test-") as tmpdir:
        ...
        self.assertEqual(result.returncode, 0, msg=result.stdout)
        self.assertIn("--run_name lookback60_hold30_daily_pg_balanced_sh90_5ep", result.stdout)
        self.assertIn("--controller_init_exit_bias -2.5", result.stdout)
        self.assertIn("--controller_max_switch_penalty_coef 0.01", result.stdout)
        ...
        self.assertNotIn("--controller_eval_diagnostics", result.stdout)
        self.assertNotIn("--controller_eval_diag_thresholds", result.stdout)
